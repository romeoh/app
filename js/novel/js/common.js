var  apiurl = '//gaeyou.com/novel/api/v1/'
	,hash
	,admin = localStorage.getItem('admin') == 'romeoh' ? admin = true : admin = false
	,member
...
function getHotList() {
    ...
    request(code+'_get_list_hot', databody, function(result){
        ...
        if (result.length < hotTotal) {
            ...
            str += '<li class="more" data-morehot=""><button class="btn btn-primary btn-block">급상승 인기소설 더 불러오기</button></li>';
        }
        */
        $('#hotList').html( $('#hotList').html() + str.join('') )
        // 더 불러오기
        $('[data-morehot]').on('click', function(evt, mp){
            //mp.remove();
            getHotList();
        })
    })
...
function initBanner() {
	if ($.browser().device != 'pc') {
		$('#makeTest').css('display', 'none');
		$('#kakaoMini').css('display', 'block');
	}
}

function initNavi() {
	if ($('[data-global]').selector.length > 0) {
		var  str = ''
			,page = $('[data-global]').data('global')
		...
		str += '</ul>';
		//str += '<div></div>';
		$('[data-global]').html(str);
		$('[data-page="' + page + '"]').addClass('on');
	}
}

// 이름과 카카오 아이디 설정
function initUserName() {
	if ($('[data-uname]').selector.length > 0) {
		var uname = localStorage.getItem('uname') || '';
		$('[data-uname]')
			.val(uname)
			.on('blur', function(evt, mp){
				localStorage.getItem('uname', $('[data-uname]').val())
			})
	}
	if ($('[data-kasid]').selector.length > 0) {
		var uname = localStorage.getItem('kasid') || '';
		$('[data-kasid]')
			.val(uname)
			.on('blur', function(){
				localStorage.setItem('kasid', $('[data-kasid]').val())
			})
	}
}
...
function switchLogin(hash) {
	...
	$('#loginform').html(str);
	$('#cancelLogin').on('click', cancelLogin);
	$('#switchJoin').on('click', function(){
		switchJoin(hash);
	});
...
function switchJoin(hash) {
	...
	$('#loginform').html(str)
	$('#cancelLogin').on('click', cancelLogin);
	$('#switchLogin').on('click', switchLogin);
	
	// 아이디 체크
	$('#uid').on('focus', function(evt, mp){
		validuid = false;
		$('#groupid')
	...
	$('#uid').on('blur', function(evt, mp){
		...
		if ($(this).val().length < 4) {
			$('#idinfo').html('4글자 이상 입력해주세요.');
			$('#groupid').addClass('has-error');
			return false;
		}

		if(!pattern.test($(this).val())){
			$('#idinfo').html('아이디는 한글로 입력할 수 없습니다.');
			$('#groupid').addClass('has-error');
			return false;
		}
	...
	// 아이디 체크
	$('#nickname').on('focus', function(evt, mp){
		validnickname = false;
		$('#groupnickname')
			.removeClass('has-error')
			.removeClass('has-success')
	...
	$('#nickname').on('blur', function(evt, mp){
		var  tr = 'member_checkid'
			,data = {}
		...
		data['uid'] = $('#nickname').val();
		data['flag'] = 'nickname';
		
		request(tr, data, function(result){
			var result = JSON.parse(result)
			if (result['result'] == '0') {
				$('#nicknameinfo').html('사용할수 있는 닉네임 입니다.');
				$('#groupnickname')
					.addClass('has-success')
					.removeClass('has-error');
				//if ($('#nickname').val() == '') {
				...
				$('#nicknameinfo').html('사용할수 없는 닉네임입니다.');
				$('#groupnickname')
					.addClass('has-error')
					.removeClass('has-success')
			}
	...
	$('#upw').on('blur', function(evt, mp){
		if ($(this).val().length == 0) {
			return false;
		}
		if ($(this).val().length < 4) {
			$('#pwinfo').html('비밀번호는 4글자 이상 입력해주세요.');
			$('#grouppw').addClass('has-error');
			return false;
		}
		$('#grouppw').addClass('has-success');
		$('#pwinfo').html('사용할수 있는 비밀번호입니다.');
		validupw = true;
	})
	$('#upw2').on('focus', function(evt, mp){
		validupw2 = false;
		$('#grouppw2')
			.removeClass('has-error')
			.removeClass('has-success')
	...
	$('#upw2').on('blur', function(evt, mp){
		if ($(this).val().length == 0) {
			return false;
		}
		if ($('#upw').val() != $('#upw2').val()) {
			$('#pwinfo2').html('비밀번호가 다릅니다.');
			$('#grouppw2').addClass('has-error');
			return false;
		}
		$('#pwinfo2').html('사용할 수 있습니다.');
		$('#grouppw2').addClass('has-success');
		validupw2 = true;
	})
	$('#btnJoin').on('click', function(){
		...
		if (!emailVerification($('#email').val())) {
			alert('이메일을 잘못되었습니다.');
			$('#email').focus();
			return false;
		}
...
function setLogin(result) {
	...
	localStorage.getItem('member', JSON.parse(result));
	return result;
}
	...
	if (!!member) {
		...
		if (dateRage == '0') {
			...
			request(tr, data, function(result){
				...
				$('#replyName').html('<i class="fa fa-user"></i> ' + member['nickname'] + ' <i class="gae-level" id="gaeLevel">' + member['level'] + '일 연속 출첵중입니다. [?]</i>')
			})
		} else {
			var  tr = 'member_chulcheck'
				,data = {}
...
function checkLogin() {
	...
	if (!!member) {
		...
		// 로그인전 댓글
		/*$('#btnReg').on('click', initLogin);
		$('#inpReply').on('focus', function(evt, mp){
			$(this).blur();
			initLogin()
		});*/
		// 썰픽 쓰기
		$('#btnfReg').on('click', initLogin);
		$('#fcontent').on('focus', function(evt, mp){
			if (!member) {
				alert('로그인이 필요합니다.')
				var search = location.search
				window.location.href = '../member/login.html#/novel/' + search
			}
			//$(this).blur();
			//initLogin()
		});
...
/*function initReplyBox() {
	...
	if ($('[data-replybox]').selector.length > 0) {
		str += '<div class="kasid">';
		...
		$('[data-uname]')
			...
			.val( localStorage.getItem('uname') || '' )
		...
		$('[data-kasid]')
			...
			.on('blur', function(evt, mp){
				localStorage.setItem('kasid', $(this).val())
				hideReply();
			})
			.val( localStorage.getItem('kasid') || '' )
		
		// 댓글 180자 제한
		$('#inpReply').on('keyup', function(evt, mp){
			var txt = mp.html()
			if (txt.length >= 180) {
		...
		// 댓글쓰기 전문통신
		$('#btnSubmit').on('click', function(evt, mp){
			if ($('#inpReply').html() == '') {
				alert('내용을 입력해주세요.');
				$('#inpReply').focus();
				return false;
			}
			if ($('#inpReply').hasClass('msg')) {
				alert('내용을 입력해주세요.');
				$('#inpReply').focus();
				return false;
			}
			...
			if ($('[data-uname]').val() == '') {
				alert('이름을 입력해주세요.');
				$('[data-uname]').css('display', 'block');
				$('[data-kasid]').css('display', 'block');
				$('[data-uname]').focus();
				return false;
			}
			localStorage.setItem('uname', localStorage.getItem('uname'));
			localStorage.setItem('kasid', localStorage.getItem('kasid'));
			
			imoidx = process(dataPhoto);
			imoticon = dataPhoto[imoidx]
			bodyData = {
				'idx': cuData['idx'],
				'uname': encodeURIComponent($('[data-uname]').val()),
				'kasid': encodeURIComponent($('[data-kasid]').val()),
				'photo': encodeURIComponent(imoticon),
				'text': encodeURIComponent($('#inpReply').html()),
				'ua': navigator.userAgent
			}
			$.ajax({
				 'url': apiurl + code + '_reply_add.php'
...
function hideReply() {
	$('[data-uname]').css('display', 'none');
	$('[data-kasid]').css('display', 'none');
}
...
function initMenu() {
	...
	function showGnb() {
		...
		str += '		<dd><a href="//romeoh.github.io/kakaoStory/html/makeme.html"><span class="ico arrow"></span>깨알테스트</a></dd>';
		...
		$('#gnb').html(str);
		//if (uname) {
		//	$('#user').html(uname + '님, 안녕하세요.')
		//}
		$('#gnb').css('height', $('body').css('height'))
...
function shareData(_obj, _opt) {
	...
	if (media == 'facebookSharer') {//
		...
		str += '//www.facebook.com/sharer.php';
		str += '?s=100';
	...
	if (media == 'facebook') {//API
		...
		$('#facebookScript').on('load', function(evt, mp){
			var  obj = _obj || {}
				,mode = obj.mode || 'real'
				,feed = obj.feed || 'feed'
