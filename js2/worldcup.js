...
function ready() {
	...
	// 인기테스트 타이틀 변경
	if (listFlag == 'hot') {
		/*M('#hotListTitle').html('급상승 인기 월드컵')
		M('#hotListOrder')
	...
	$.ajax({
		...
		,'success': function(result){
			...
			for (var i=0; i<result.length; i++) {
				...
				str += '			<em class="dsc"><span class="s' + result[i]['total'] + '">' + result[i]['total'] + '강전</span> | 조회: ' + M.toCurrency(result[i]['view']) + '회 | 경기수: ' + M.toCurrency(result[i]['excute']) + '회 | ' + decodeURIComponent(result[i]['uname']) + '</em>';
				str += '		</span>'
				str += '	</a>';
				str += '</li>';
			}
			if (result.length < hotTotal) {
				...
				str += '<li class="more" data-morehot>신규 월드컵 더 불러오기</li>';
			}
	...
	$.ajax({
		...
		,'success': function(result){
			...
			if (result.length < pageTotal) {
				...
				str += '<li class="more" data-morenew>최근 월드컵 더 불러오기</li>';
			}
...
function getScore(flag) {
	...
	$.ajax({
		...
		,'success': function(result){
			...
			str += '</dl>';
			str += '<a href="add.html">월드컵을 직접 만들어 보세요.</a>';
			M('#tinfo').html(str);
		}
	})
}
...
function initEvent() {
	// 카스로 보내기
	M('#btnKakaoStory').on('click', function(){
		var post = ''
